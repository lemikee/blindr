...
function ChatScreen(props) {
  ...
  if (!localStorage.messagesGitlab) {
    defaultMessages = [
      {
        ...
        message: "Hi, we have a merge conflict and heard you're the person to help!",
      },
      {
        message:
          "I can totally help!",
    ...
    localStorage.messagesGitlab = JSON.stringify(defaultMessages);
    // console.log("DNE")
  }

  const [messagesGitlab, setMessages] = useState(JSON.parse(localStorage.messagesGitlab));

  useEffect(() => {
    // uses localStorage for persistant messages upon refresh
    localStorage.messagesGitlab = JSON.stringify(messagesGitlab);
    if (props.dropdown) {
      let number =
  ...
  const handleSend = (e) => {
    e.preventDefault();
    if (input !== "") {
      setMessages([...messagesGitlab, { message: input }]); // adds message from input to messages array on ln 10
